...
class Fasta:
    ...
    def __init__(self, file_path):
        ...
        fasta_path = (
            f"{dir_path}/fasta.txt"  # FASTA格式的文件，但信息行和核酸序列顺序颠倒
        )
        ...
        reduced_path = (
            f"{dir_path}/reduced_gene_info.txt"  # 存储gene相关信息，且去除重复行
        )

        nucleic_acid = self.nucleic_acid_list(
            filter_path
        )  # 用于按顺序保存所有核酸信息，一个base作为一个元素
            ...
            with open(path2, "w") as fas:
                ...
                for line in f:
                    ...
                    if search_locus:
                        ...
                        info.append(
                            f"\t[organism={self.organism}]\t[chromosome={self.chromosome}]"
                        )
                        info_line = "".join(
                            info
                        )  # 更新信息行中的locus，organism，chromosome信息
                        fas.write("\n" + info_line + "\n")  # 信息行写入
